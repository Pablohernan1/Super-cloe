...
export function QuickSearch() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const [{ data: creditLimit }, { data: guarantors }, { data: loans }, { data: overdue }] = await Promise.all([
        ...
        supabase.from('loans').select('id').eq('customer_id', customer.id).eq('status', 'active'),
        supabase
          .from('installments')
        ...
        {result && (
          <div className="rounded-lg border p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <Link href={`/clientes/${result.customer.id}`}>
                <Button variant="outline" size="sm">
                  <User className="mr-2 h-4 w-4" />
                  Cuenta
                </Button>
              </Link>
              <Link href={`/prestamos/simulacion?customer_id=${result.customer.id}`}>
                <Button variant="outline" size="sm">
                  <FileText className="mr-2 h-4 w-4" />
                  Nuevo préstamo
                </Button>
              </Link>
              <Link href={`/cobranza?customer_id=${result.customer.id}`}>
                <Button variant="outline" size="sm">
