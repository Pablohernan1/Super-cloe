...
export function CreditLimitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Error al crear límite de crédito')
      }

      router.push(`/creditos/${data.id}`)
    } catch (err) {
          ...
          <FieldGroup>
            ...
            {customer && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="text-sm font-medium text-blue-900">{customer.first_name} {customer.last_name}</div>
                <div className="text-xs text-blue-700">{customer.customer_code} - {customer.cuit_cuil}</div>
              </div>
            )}
          </FieldGroup>
