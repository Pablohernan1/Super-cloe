  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      setAlerts(
        rawAlerts.map((a) => ({
          ...a,
          loan: a.reference_id ? loansById[a.reference_id] : undefined,
        }))
      )
    } catch (error) {
        ...
        <CardContent className="pt-6 flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">
              Morosos, vencidos, bloqueados y garantes comprometidos
            </p>
                  ...
                  filtered.map((alert) => (
                    <TableRow key={alert.id}>
                      <TableCell className="font-medium">
                        {alert.loan?.customer
                          ? `${alert.loan.customer.first_name} ${alert.loan.customer.last_name}`
                          : '-'}
                      </TableCell>
                      ...
                      <TableCell>
                        {alert.loan?.customer?.status === 'blocked' ? (
                      ...
                      <TableCell className="text-right">
                        {alert.loan && (
                          <Link href={`/prestamos/${alert.loan.id}`}>
